...
class AdminController extends Controller
{
    ...
    // Check if the brand has bikes or not
    // Brand can't be deleted if it has bikes
    function checkBrand(Brand $brand)
    {
    }

    function removeBrand(Brand $brand)
    {
        $findBrand = Brand::findOrFail($brand->id);

        // Check bikes 
        $bikesFound = Bike::where('brand_id', $findBrand->id)->exists();

        if ($bikesFound) {
            return back()->with('error', 'Can\'t delete brand, one or more bikes are registered with this brand.');
        } else {
            // Delete the brand
            $brand->delete();
